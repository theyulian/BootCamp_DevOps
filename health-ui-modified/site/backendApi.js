...
function getPatientInfo(url, patientID) {
	return new Promise(function(resolve, reject) {
		request(url + "resources/v1/getInfo/patients/" + patientID, function (error, response, body) {
...
function getPatientMedications(url, patientID) {
	return new Promise(function(resolve, reject) {
		request(url + "resources/v1/getInfo/prescription/" + patientID, function (error, response, body) {
			var medications = [];
	 		if (!error && response.statusCode == 200) {
	 			body = JSON.parse(body);
...
function getPatientAppointments(url, patientID) {
	return new Promise(function(resolve, reject) {
		request(url + "resources/v1/appointments/list/" + patientID, function (error, response, body) {
			var appointments = [];
	 		if (!error && response.statusCode == 200) {
...
function getPatientMeasurements(url, patientID) {
	return new Promise(function(resolve, reject) {
		request(url + "resources/v1/listObs/" + patientID, function (error, response, body) {
			var measurements = {};
	 		if (!error && response.statusCode == 200) {
	 			body = JSON.parse(body);
	 					...
	 					case "Systolic Blood Pressure":
	 						if (!measurements.sys) {
	 							measurements.sys = measurementsData[i]["NUMERICVALUE"];
                                                                measurements.date = measurementsData[i]["DATEOFOBSERVATION"];
	 						}
	 						break;
...
function patientLogin(url, username, password) {
	return new Promise(function(resolve, reject) {
		request.post({url: url + "resources/v1/login/user", json: {"UID":username,"PASS":password}}, function (error, response, body) {
			if (!error && response.statusCode == 200) {
